...
function getProjectPaths() {
  // 環境変数 EXCEL_PROJECT_ROOT から取得（必須）
  const projectRoot = process.env.EXCEL_PROJECT_ROOT;

  if (!projectRoot) {
    console.error('[Excel] エラー: 環境変数 EXCEL_PROJECT_ROOT が設定されていません');
    throw new Error('EXCEL_PROJECT_ROOT environment variable is not set');
  }
...
async function getExcelFile(dateStr) {
  ...
  if (files.length === 0) {
    return { success: false, error: `テンプレートフォルダにxlsxファイルがありません: ${templateDir}` };
  }
  const templatePath = path.join(templateDir, files[0]);
