...
export async function findExcelRow(dateStr, customerName, visitCount) {
  try {
    ...
    console.error('Excel検索エラー:', error);
    return {
      success: false,
      error: error.message
    };
...
export async function deleteExcelRow(dateStr, customerName, visitCount) {
  try {
    ...
    if (!findResult.success) {
      // 見つからない場合はエラーではなく成功扱い（既に削除されている可能性）
      //console.log('削除対象の行が見つかりません（既に削除済みの可能性）');
      return {
        success: true,
    ...
    console.error('Excel行削除エラー:', error);
    return {
      success: false,
      error: error.message
    };
...
export async function updateExcelRow(oldData, newData) {
  try {
    ...
    if (!findResult.success) {
      // 見つからない場合は新規追加
      //console.log('更新対象の行が見つからないため、新規追加します');
    ...
    console.error('Excel行更新エラー:', error);
    return {
      success: false,
      error: error.message
    };
